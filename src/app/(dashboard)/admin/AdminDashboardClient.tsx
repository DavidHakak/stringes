...
import { 
  ...
  Plus, Check, Loader2, ListFilter, Users, RefreshCw, X
} from 'lucide-react';
...
export default function AdminDashboardClient({ initialUsers }: { initialUsers: MappedUser[] }) {
  ...
  // Load Whitelist when selected user changes
  useEffect(() => {
    if (!selectedUser) {
      setWhitelist([]);
      return;
        ...
        {selectedUser ? (
          <div className="bg-card/40 border border-border/80 rounded-2xl p-6 shadow-md backdrop-blur-md animate-fade-in">
            <div className="border-b border-border pb-4 mb-6">
              <h3 className="text-lg font-bold text-foreground">סינון ערוצים</h3>
              <p className="text-xs text-muted-foreground mt-0.5">ערוצי YouTube המאושרים ל: <strong className="text-primary font-semibold">{selectedUser.name}</strong></p>
            </div>
